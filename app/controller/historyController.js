app.controller(
  ...
  function (
    ...
    $scope.openPrintModal = () => {
      $scope.printData = {
        name: selectedInvoice.supplier_name
    ...
    $scope.print = function () {
      ...
      if ($scope.printData.currency == "dollar") {
        ...
        $scope.printData.total =
          $scope.totalPrice * stockModel.exchangeRate.exchange_rate;
      }
      // }
      // ipcRenderer.send("printDocument", [invoice, $scope.printData]);
      window.electron.ipcRenderer.invoke("print-invoice",  [invoice, $scope.printData]);
    };
  }
);
