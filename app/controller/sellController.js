  ...
  function (
    ...
    $scope.print = async function () {
      if ($scope.invoice.length > 0) {
        ...
        await window.electron.ipcRenderer.invoke("print-invoice",  [$scope.invoice, $scope.printData]);
    ...
    $scope.selectOption = function (event) {
      if (event.keyCode == 13 && $scope.selectedItem) {
        ...
        if (count == 0) {
          NotificationService.showErrorText(
            "Item not found, it is not defined!"
          );
        }
    ...
    $scope.cancelOrder = function () {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          angular
            .element(document.querySelector("#barcodeInput"))
            .trigger("focus");
        }
      });
    };
